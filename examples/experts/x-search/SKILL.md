...
name: x-search
description: >
  X/Twitter Latest 搜索专家。用本地已登录 Chrome(CDP)按关键词 + [startIso, endIso)
  时间窗抓推文,双边界过滤,分块 dump 全量原文不截断,落地结构化 JSON。
  当需要从 X/Twitter 按时间范围批量取推文时用这个专家。
---
...
CDP 的授权由网关在 spawn 你之前经 `UGK_TASK_ALLOW_CHROME_CDP` 注入,**不要**写 `chrome_cdp status` 检查 → `chrome_cdp launch` → 重试。进来直接用 `chrome_cdp navigate/evaluate` 即可。隔离 tab 已由网关分配,不用指定 target。

## 输入(全部已由调用方算好,你直接用)

从 `TASK_INPUT`(JSON,环境变量已注入)读,都是扁平标量字段,不是嵌套对象:
...
**你不解析时间、不调时间解析脚本。** 调用方(main agent)是知道当前时间的 LLM,已经把"俩月/上周/last week"算成 startIso/endIso 了。

**输入校验(开 Chrome 前必做):** 上述必填字段必须全部存在且是标量(string/number)。**若任一缺失或不是标量,直接报错退出,不要开 Chrome,不要现编默认时间值。** 这是输入错误的信号,现编会产出错误结果。

### 组装 timeWindow 对象(写输出时用)

把扁平字段组装成 timeWindow 对象,写进输出 JSON:
```json
...
所有脚本在 `$TASK_DIR/scripts/`(环境变量 `TASK_DIR` 已注入)。**读脚本内容传给 `chrome_cdp evaluate`** —— 不要复刻脚本逻辑。
...
`hasArticles > 0` 再继续。若 `title`/页面文本显示登录墙、限流、错误,按 stopReason 上报,别硬滚。
...
注意:所有字符串要安全 JSON stringify 后再拼表达式,不要手写引号导致关键词或中文破坏 JS。
...
返回值是摘要 + 预览(小):`stopReason, cutoffReached, score, grade, anchorScrolls, rowsInspected, validRows, filteredRows, rows(预览50条)`。全量定型结果已存在页面的 `window.__xSearcherLastResult`。
...
把扁平字段组装成的 timeWindow 对象、第 5 步的 benchmark、第 6 步累积的完整 results,拼成下面结构,`fs.writeFileSync` 到 `$TASK_OUTPUT_DIR/x_search_results.json`:
...
```json
{
  ...
  "method": "x-search expert / DOM fallback with MutationObserver + anchor-overlap scrolling / local Chrome CDP",
  "benchmark": {
    "stopReason": "<第5步返回>", "cutoffReached": <bool>, "score": <N>, "grade": "<A-D>",
    "anchorScrolls": <N>, "maxSteps": <N>, "rowsInspected": <N>, "validRows": <N>,
    "filteredRows": <N>, "rowsReturned": <results.length>, "validRate": <0-1>,
    "keywordMatchRate": <0-1>, "totalRunMs": <N>
  },
...
- maxSteps 到顶但没到 cutoff:照写,benchmark.grade 偏低,标 partial。调用方可重跑提高 maxSteps。
